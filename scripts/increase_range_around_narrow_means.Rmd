...
predplot <- ggplot(prediction, aes(x = MEAN, y = fit, color = COVER_TYPE)) +
  ...
  xlim(0,50) + ylim(0,30) +
  labs(x = NULL, y = NULL, subtitle  = 'Linear Model Predictions', title = NULL) +
  scale_color_manual('Functional Group', values = lifeform_pal) +
...
finalplot <- final_values %>%  #  something like this should convey the idea of the lower left quad being off... 
  ...
  geom_jitter() +
  theme_bw() +
  labs(x = NULL, y = NULL, title = NULL, subtitle  = 'Final') +
  xlim(0,50) + ylim(0,30) +
  scale_color_manual('Functional Group', values = lifeform_pal) +
...
patchwork::wrap_plots(rawplot, predplot, finalplot, legend,  ncol=1, heights=c(1, 1, 1, 0.3) ) +
