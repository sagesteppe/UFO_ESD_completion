...
This simple model served to explain a moderate amount ($r^2$  = `r round(broomSUM$r.squared,2)`) of the variation in the *response variables...*. It found that functional type has a very strong effect on the slope of the trend line (p = `r round(anovaTABLE$p[2], 4)`), and there was strong evidence (p = `r round(anovaTABLE$p[1], 4)`) that increasing the mean of the values had a negative effect on the observed variation. For every one percent increase in the mean of the benchmark values the percent range in variation of the estimates decreased by `r round(model$coefficients[['MEAN']], 3)` percent. 
...
final_values <- prediction %>% 
  group_by(ECO.SITE, COVER_TYPE) %>%  # do it do it do it
  mutate(RANGE = round(fit, 0)) %>% 
  mutate(COVER = if_else(ESTIMATE == 'LOWER', MEAN - (RANGE/2), MEAN + (RANGE/2)))
  
  select(ECO.SITE:COVER) %>% # need to make same format for lower and upper 
