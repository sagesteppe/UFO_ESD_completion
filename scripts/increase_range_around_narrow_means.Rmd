...
\begin{wraptable}{l}{53mm}
    ...
    \end{tabular}
    \caption{* calculated as the \textit{range} divided by the midpoint of the \textit{mean} multiplied by 100 to gather a percentile $\frac{max(Range)} {\text{midpoint( Mean)}} * 100$ }
    \label{Expected Variation of Benchmarks}
\end{wraptable}

While the approach described above is prudent in the development of an ESD, it is not prudent for us to assume such narrow ranges of variation. Ranges this narrow may unduly penalize estimates of the amount of land under analysis which are meeting condition benchmarks. This can lead to a misleading assessment of land health conditions.
...
\begin{wrapfigure}{r}{0.4\textwidth}
  \centering
    \includegraphics[width=0.4\textwidth]{../results/graphics/Benchmark_Ranges.png}
...
Ranges of estimated benchmark variation were estimated as being too low if they fell within the ranges in Table 1 & Figure 1 *top panel*. These `r nrow( distinct(predict_data, ECO.SITE, COVER_TYPE) )` values were removed from the initial data set. The remaining `r nrow( distinct(keep_ranges, ECO.SITE, COVER_TYPE) )` observations were used as **training** data for the linear model:  $\text{lm(Range} \sim  \text{Mean + Functional Group)}$. We believed that the variation associated with each measurement of range would decrease as the mean cover increases. In other words, as the mean of the cover estimate get's larger, the percent variation of the range of them decrease. 
