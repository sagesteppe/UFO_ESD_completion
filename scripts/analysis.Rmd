...
ordered <- read.csv(file.path(pp, f[grep('Ordered', f )]))
...
filter(transcription, UFOPlots >= 1) # 34

# how many of these ES are completed 
filter(transcription, UFOPlots >= 1 & VegMarks.Taxa == 'DONE') # 17 are done , 17 are missing

# how many of the plots do these represent
filter(transcription, UFOPlots >= 1 & VegMarks.Taxa == 'DONE') %>% summarize(PlotESD = sum(UFOPlots)) # 97 plots have ESD's
filter(transcription, UFOPlots >= 1) %>% summarize(PlotsTotal = sum(UFOPlots)) # 121 plots total have been completed.
...
UFOcal <- filter(transcription, UFOPlots >= 1) %>% select(EcologicalSiteId:VegMarks.Taxa)


stateTransition <- ordered %>% 
  distinct(ESD.CODE) %>% 
  drop_na() %>%  # need to fix this..
