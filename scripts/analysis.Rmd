...
4) does vegetation community production table exist. 
 a. which states
5) do quantitative plant cover benchmarks exist?
...
# quantity <- replace(quantity, quantity < 0,0) don't do this... 
...
nodes <- data.frame(
  ID = c('Base Plots', 'Sampled', '', 'ES', '', 'ESD',  '',
          'Similar', '', 'Benchmarks', 'N.Benchmarks'),
  label = c('Base Plots', 'Sampled', '', 'ES', '', 'ESD',  '',
          'Similar', '', 'Benchmarks', 'Benchmarks'),
  x = c(1, rep(2:6, each = 2)), y = c(2, rep(c(2,1), times = 5)),
  label_pos = c('above', rep(c('above', 'below'), times = 5))
...
df <- data.frame(
  from = c('BP', 'OS', 'OS', 'ES', 'ES', 
           'N.ES', 'ESD', 'ESD', 'ESD', 'N.ESD',
           'SIMILAR', 'N.SIMILAR'),
  to = c('OS', 'ES', 'N.ES', 'ESD', 'N.ESD',
         'N.ESD', 'SIMILAR', 'N.SIMILAR', 'N.SIMILAR', 
         'BM', 'N.BM', 'N.BM'), 
  quantity = c(281, 237, 44, 190, 47,
               44, 180, 100, 91, 82,
               99, 101
               )
)
